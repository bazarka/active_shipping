...
class LTLTest < Test::Unit::TestCase
  ...
  def test_initialize_options_requirements
    assert_raises(ArgumentError) { LTL.new }
    assert_raises(ArgumentError) { LTL.new(:login => 'blah', :password => 'bloo') }
    assert_raises(ArgumentError) { LTL.new(:login => 'blah', :key => 'kee') }
    assert_raises(ArgumentError) { LTL.new(:password => 'bloo', :key => 'kee') }
    assert_nothing_raised { LTL.new(:loginId => 'blah', :password => 'bloo', :licenseKey => 'kee', :accountNumber => 'account') }
  end
  ...
  def test_find_tracking_info_should_return_correct_carrier
    @carrier.expects(:commit).returns(@tracking_response)
    assert_equal :ltl, @carrier.find_tracking_info('1Z5FX0076803466397').carrier
  end
